...
Obrigado por se interessar por contribuir no projeto :shark:`sharknado`:tornado:. Nesse documento estão listadas as operações mais comuns e que você pode precisar para contribuir.
...
O projeto é dividido em três diretórios. `docs`, `sharknado` e `tests`. Onde cada diretório tem sua função especifica.
...
flowchart
	...
	sharknado --> previsao.py
	sharknado --> cli.py
```

O código do CLI e da biblioteca estão em `sharknado`. A documentação da API do código também está sendo feita em `sharknado`. Com uso da ferramenta [mkdocstrings](https://mkdocstrings.github.io/) e seguem o padrão de [docstrings do google](https://sphinxcontrib-napoleon.readthedocs.io/en/latest/example_google.html). Então, caso altere alguma coisa no código, lembre-se de atualizar também as docstrings.
...
```py
cidade, unidade, grafico = funcao('arg1', 'arg2','arg3').values()
```
...
Coisas importantes que você deve saber sobre os testes é que nem todos os testes estão somente no diretório `sharknado/tests`. A flag de `addopts = "--doctest-modules"` está sendo usada. Então, caso modifique algo, esteja ciente que as docstrings também rodam testes e são a base para documentação da API, então tome cuidado com alterações.
...
```bash
poetry run sharknado 'nome_da_cidade'
```
...
{% include "todos/apitodo.md" %}


## Não achei o que preciso aqui

Caso não tenha encontrado o que precisa, você pode abrir uma [issue no projeto](https://github.com/kmaximo/sharknado_weather/issues) relatando o que não consegue fazer ou o que precisa ser melhor documentado
